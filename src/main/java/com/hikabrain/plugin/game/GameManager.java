...
public class GameManager {
     ...
     * Met aussi à jour l'état tout de suite en fonction de cette config : sans ça, l'arène
     * restait bloquée sur NOT_CONFIGURED (sa valeur par défaut) jusqu'à ce qu'un premier joueur
     * la rejoigne puis la quitte, ce qui la faisait apparaître à tort comme "non configurée"
     * dans le GUI/la commande /hb list juste après un redémarrage du serveur.
     */
    ...
    /**
     * Recalcule l'état de l'arène à partir de sa configuration actuelle (lobby, spawns,
     * zones de capture). À appeler après toute commande admin qui modifie cette
     * configuration, pour que l'arène apparaisse immédiatement disponible (verte) dans le
     * GUI/les commandes dès qu'elle devient entièrement configurée — sans avoir besoin
     * qu'un joueur la rejoigne puis la quitte pour "forcer" la mise à jour.
     *
     * Ne touche jamais à l'état pendant qu'une partie est en cours, en préparation ou en
     * train de se terminer (COUNTDOWN/STARTING/PLAYING/ROUND_RESET/ENDING) : seuls les
     * états NOT_CONFIGURED et WAITING peuvent être recalculés ici.
     */
    public void refreshConfigurationState() {
        if (state == GameState.NOT_CONFIGURED || state == GameState.WAITING) {
            state = arena.isFullyConfigured() ? GameState.WAITING : GameState.NOT_CONFIGURED;
        }
    }
