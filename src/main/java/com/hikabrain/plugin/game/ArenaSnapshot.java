...
public class ArenaSnapshot {
    ...
    public void capture(CuboidRegion region) {
        ...
        if (total > 2_000_000) {
            logger.warning("La zone de jeu HikaBrain est très grande (" + total
        ...
        logger.info("Snapshot de la zone de jeu HikaBrain capturé (" + blockDataStrings.length + " blocs).");
    }
    ...
    public boolean loadFromFile(File file) {
        ...
        try (DataInputStream in = new DataInputStream(new BufferedInputStream(new FileInputStream(file)))) {
            ...
            if (loadedWorld == null) {
                logger.warning("Le monde '" + worldName + "' du snapshot HikaBrain n'existe pas (encore).");
                return false;
            }
