...
public class PlayerMoveListener implements Listener {
    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        // Fin de partie (écran de victoire) : les joueurs sont passés en spectateur mais
        // n'ont pas encore été téléportés au lobby (léger délai configurable). Sans ça, ils
        // pourraient s'envoler hors de l'arène en mode spectateur avant ce transfert. On les
        // confine donc exactement comme un spectateur classique (mêmes limites), en les
        // ramenant au centre de l'arène s'ils tentent d'en sortir.
        if (gm.getState() == GameState.ENDING) {
            if (!isSameBlock(event.getFrom(), event.getTo()) && !gm.isWithinSpectatorBounds(event.getTo())) {
