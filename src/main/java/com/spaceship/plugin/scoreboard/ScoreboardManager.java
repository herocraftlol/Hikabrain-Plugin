...
public class ScoreboardManager {
    ...
    public void loadConfig() {
        title = plugin.getSpaceConfig().getString("scoreboard.title", "&8[&b&lHEROCRAFT&8] &6&lSpaceShip");
    }
    ...
    private Scoreboard createPlayerScoreboard(Player player, GameManager gm) {
        ...
        // Ajouter les lignes du sidebar : vue spectateur (points/salle de tous les joueurs)
        // ou vue joueur (K/D personnel), selon le statut du joueur sur cette arène.
        if (gm.isSpectating(player)) {
            addSpectatorSidebarLines(board, objective, gm);
        } else {
    ...
    private void addPlayerSidebarLines(Scoreboard board, Objective objective, Player player, GameManager gm) {
        ...
        String elapsed = formatElapsed(gm.getElapsedSeconds());

        // Lignes du sidebar : timer + progression de la partie + effectifs + K/D du joueur
        String[] lines = {
            "&6&lSpaceShip",
        ...
        writeSidebarLines(board, objective, lines);
    }

    /** Nombre maximum de joueurs affichés par équipe dans le classement spectateur (voir ci-dessous). */
    private static final int SPECTATOR_MAX_PLAYERS_PER_TEAM = 5;

    /**
     * Construit les lignes du sidebar pour un SPECTATEUR (pas de K/D personnel puisqu'il ne
     * joue pas) : la salle où se trouvent actuellement les joueurs (pour se repérer après une
     * téléportation automatique), la progression le long du vaisseau, et les points (kills)
     * de chaque joueur, triés par équipe puis par nombre de kills décroissant.
     */
    private void addSpectatorSidebarLines(Scoreboard board, Objective objective, GameManager gm) {
        com.spaceship.plugin.game.Team BLACK = com.spaceship.plugin.game.Team.BLACK;
        com.spaceship.plugin.game.Team WHITE = com.spaceship.plugin.game.Team.WHITE;

        String progressBar = buildProgressBar(gm);
        String roomLabel = gm.getCurrentRoomLabel();
        String elapsed = formatElapsed(gm.getElapsedSeconds());

        List<String> lines = new ArrayList<>();
        lines.add("&6&lSpaceShip &7(Spectateur)");
        lines.add("&7" + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500);
        lines.add("&e\u23f1 Temps: &f" + elapsed);
        lines.add("&eSalle: &f" + roomLabel);
        lines.add("&fVaisseau: " + progressBar);
        lines.add("&7" + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500 + (char) 0x2500);

        lines.add("&8\u2764 &8&lNoir");
        addTeamPlayerLines(lines, gm, BLACK);

        lines.add("&f\u2764 &f&lBlanc");
        addTeamPlayerLines(lines, gm, WHITE);

        // Le scoreboard n'affiche proprement qu'environ 15-16 lignes : on tronque si besoin
        // plutôt que de laisser Minecraft couper l'affichage de façon imprévisible.
        List<String> truncated = lines.size() > 16 ? lines.subList(0, 16) : lines;
        writeSidebarLines(board, objective, truncated.toArray(new String[0]));
    }

    /**
     * Ajoute, pour l'équipe donnée, une ligne "Nom: kills" par joueur (triés par kills
     * décroissants), plafonné à {@link #SPECTATOR_MAX_PLAYERS_PER_TEAM} pour ne pas
     * déborder le sidebar quand beaucoup de joueurs sont en partie ; le reste est résumé
     * par une ligne "+N de plus".
     */
    private void addTeamPlayerLines(List<String> lines, GameManager gm, com.spaceship.plugin.game.Team team) {
        List<Map.Entry<UUID, com.spaceship.plugin.game.Team>> members = new ArrayList<>();
        for (Map.Entry<UUID, com.spaceship.plugin.game.Team> e : gm.getPlayerTeams().entrySet()) {
            if (e.getValue() == team) members.add(e);
        }
        members.sort((a, b) -> Integer.compare(gm.getPlayerKills(b.getKey()), gm.getPlayerKills(a.getKey())));

        if (members.isEmpty()) {
            lines.add("&7  (aucun joueur)");
            return;
        }

        int shown = Math.min(members.size(), SPECTATOR_MAX_PLAYERS_PER_TEAM);
        for (int i = 0; i < shown; i++) {
            UUID uuid = members.get(i).getKey();
            org.bukkit.entity.Player p = Bukkit.getPlayer(uuid);
            String name = p != null ? p.getName() : "?";
            int kills = gm.getPlayerKills(uuid);
            lines.add("&7  " + name + ": &a" + kills);
        }
        if (members.size() > shown) {
            lines.add("&8  +" + (members.size() - shown) + " de plus");
        }
    }

    /**
     * Écrit un tableau de lignes dans le scoreboard donné (une "team" scoreboard par ligne,
     * avec une entrée invisible dédiée — voir {@link #invisibleEntry(int)}).
     */
    private void writeSidebarLines(Scoreboard board, Objective objective, String[] lines) {
    ...
    /**
     * Formate un nombre de secondes écoulées en "mm:ss" (ou "h:mm:ss" au-delà d'une heure),
     * pour le timer de partie affiché dans le sidebar.
     */
    ...
    private GameManager resolveGameManager(Player player) {
        ...
        return plugin.getSpaceArenaManager().findSpectatingArenaOf(player);
    }
    ...
    private void updatePlayerSidebar(Scoreboard board, Player player, GameManager gm) {
        ...
        // Recréer les lignes : vue spectateur ou vue joueur, selon le statut du joueur
        if (gm.isSpectating(player)) {
            addSpectatorSidebarLines(board, objective, gm);
        } else {
            addPlayerSidebarLines(board, objective, player, gm);
