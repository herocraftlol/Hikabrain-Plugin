...
public class GameManager {
    ...
    public GameManager(HikaBrainPlugin plugin, String arenaName) {
        ...
        File arenasDir = new File(plugin.getDataFolder(), "spaceship-arenas");
        if (!arenasDir.exists()) {
            arenasDir.mkdirs();
        }
    ...
    public int getMaxPlayers() {
        ...
        return plugin.getSpaceConfig().getInt("max-players", 16);
    }
    ...
    public boolean addPlayer(Player player) {
        ...
        plugin.getSpaceScoreboardManager().showScoreboard(player, this);

        broadcast(MessageUtil.format(plugin.getSpaceConfig().getString("messages.join", ""))
                .replace("%current%", String.valueOf(playerTeams.size()))
                .replace("%max%", String.valueOf(max)));
        MessageUtil.send(player, plugin.getSpaceConfig().getString("messages.team-assigned", "")
    ...
    public void removePlayer(Player player) {
        ...
        MessageUtil.send(player, plugin.getSpaceConfig().getString("messages.leave", ""));
        ...
        if (state == GameState.COUNTDOWN
                && (playerTeams.size() < plugin.getSpaceConfig().getInt("min-players", 2) || !bothTeamsHavePlayers())) {
            cancelCountdown();
        }
    ...
    public boolean changePlayerTeam(Player player, Team newTeam) {
        ...
        MessageUtil.send(player, plugin.getSpaceConfig().getString("messages.team-changed", "")
    ...
    /**
     * Libellé lisible de la salle actuellement active (ex: "Mid (neutre)", "Base 2 (Noir)"),
     * utilisé pour informer les spectateurs de la position actuelle des joueurs dans le
     * vaisseau (voir ScoreboardManager).
     */
    public String getCurrentRoomLabel() {
        return formatRoomLabel(getCurrentRoomId());
    }

    private String formatRoomLabel(String roomId) {
        if ("mid".equals(roomId)) {
            return "Mid (neutre)";
        }
        java.util.regex.Matcher m = java.util.regex.Pattern.compile("^base(\\d+)(black|white)$").matcher(roomId);
        if (m.matches()) {
            String num = m.group(1);
            String team = "black".equals(m.group(2)) ? "Noir" : "Blanc";
            return "Base " + num + " (" + team + ")";
        }
        return roomId;
    }
    ...
    public boolean addSpectator(Player player) {
        ...
        MessageUtil.send(player, "&7Reste à proximité : tu seras ramené automatiquement si tu t'éloignes trop de la salle en cours.");
    ...
    private void checkLobbyStart() {
        ...
        int min = plugin.getSpaceConfig().getInt("min-players", 2);
    ...
    private void startCountdown() {
        ...
        int max = plugin.getSpaceConfig().getInt("max-players", 16);
        boolean isFull = playerTeams.size() >= max;
        countdownSecondsLeft = isFull
                ? plugin.getSpaceConfig().getInt("lobby-countdown-fast", 10)
                : plugin.getSpaceConfig().getInt("lobby-countdown-min-reached", 10);

        broadcast(plugin.getSpaceConfig().getString("messages.countdown-start", "")
    ...
    private void startGame() {
        ...
        plugin.getSpaceScoreboardManager().onGameStart(this);

        broadcast(plugin.getSpaceConfig().getString("messages.game-start", ""));
    }
    ...
    // Horodatage (epoch millis) du début de la partie en cours, et nombre de percées
    // ("points") marquées depuis ce début. Utilisés pour le timer du scoreboard et pour
    // alimenter le classement des parties les plus longues (voir GameHistoryManager).
    private long gameStartTimeMillis = 0L;
    private int gameCaptures = 0;
    ...
    private BukkitTask spectatorConfinementTask;

    /**
     * Démarre la tâche qui empêche les spectateurs de s'éloigner de la salle où se
     * trouvent actuellement les joueurs (voir {@link #confineSpectators()}).
     */
    private void startSpectatorConfinement() {
        if (spectatorConfinementTask != null) {
            spectatorConfinementTask.cancel();
        }
        spectatorConfinementTask = Bukkit.getScheduler().runTaskTimer(plugin, this::confineSpectators, 4L, 4L);
    }

    private void stopSpectatorConfinement() {
        if (spectatorConfinementTask != null) {
            spectatorConfinementTask.cancel();
            spectatorConfinementTask = null;
        }
    }

    /**
     * Vérifie périodiquement (toutes les 10 ticks) que chaque spectateur est bien resté
     * dans la salle active, et le retéléporte sinon au centre de cette salle.
     * <p>
     * Si des limites précises ont été définies pour la salle courante (voir
     * {@code /ss setroombounds}), on utilise cette zone réelle (murs/plafond compris) :
     * un spectateur en dehors est retéléporté au centre géométrique de la zone. Sinon, on
     * retombe sur un simple rayon autour du point de vue calculé (voir
     * {@link #getSpectatorTeleportLocation()}).
     */
    private void confineSpectators() {
        if (spectators.isEmpty()) return;

        String roomId = getCurrentRoomId();
        CuboidRegion bounds = arena.getRoomBounds(roomId);
        ...
        // Pas de limites précises configurées pour cette salle : confinement approximatif
        // par distance au point de vue calculé.
        Location anchor = getSpectatorTeleportLocation();
        if (anchor == null || anchor.getWorld() == null) return;

        double radius = plugin.getSpaceConfig().getDouble("spectator-confinement-radius", 20.0);
        ...
        for (UUID uuid : spectators) {
            ...
            if (loc.getWorld() == null || !loc.getWorld().equals(anchor.getWorld())
                    || loc.distanceSquared(anchor) > radiusSquared) {
    ...
    private void handleZoneCapture(Team scoringTeam) {
        ...
        broadcast(plugin.getSpaceConfig().getString("messages.point-scored", "")
    ...
    private void startRoundReset() {
        ...
        roundResetSecondsLeft = plugin.getSpaceConfig().getInt("round-reset-countdown", 5);
    ...
    private void endGame(Team winner) {
        ...
        // Capturée tout de suite : la partie se termine maintenant, donc c'est la durée
        // finale de la partie (utilisée pour le classement des parties les plus longues).
        long finalDurationSeconds = getElapsedSeconds();
        ...
        plugin.getSpaceStatsManager().addWin(winner, teamSize);
        ...
        for (Map.Entry<UUID, Team> entry : playerTeams.entrySet()) {
            ...
            plugin.getSpaceStatsManager().addPlayerGameResult(uuid, pName, won, teamSize);
        }

        plugin.getSpaceLeaderboardManager().refreshAll();
        ...
        // Archive la partie dans l'historique des parties les plus longues (nom des
        // joueurs, date, kills et points marqués) — voir GameHistoryManager.
        plugin.getSpaceGameHistoryManager().recordGame(new com.spaceship.plugin.stats.GameRecord(
                arenaName,
                ...
                blackPlayers,
                whitePlayers,
                blackKills + whiteKills,
                gameCaptures
        ));
        plugin.getSpaceLongestGamesLeaderboardManager().refreshAll();
        ...
        int delay = plugin.getSpaceConfig().getInt("restart-delay", 5);
    ...
    private void resetToLobby() {
        ...
        for (UUID uuid : new ArrayList<>(playerTeams.keySet())) {
            ...
            if (player != null) {
                ...
                plugin.getSpaceScoreboardManager().removeScoreboard(player);
                Location preLobbyLocation = preLobbyLocations.remove(uuid);
                if (plugin.getSpaceConfig().getBoolean("restore-pre-lobby-location", true) && preLobbyLocation != null) {
                    player.teleport(preLobbyLocation);
                } else if (plugin.getSpaceConfig().getBoolean("teleport-to-lobby-on-end", true) && arena.getLobbySpawn() != null) {
                    player.teleport(arena.getLobbySpawn());
                }
            }
        }
    ...
    private void broadcast(String rawMessage) {
        String prefix = plugin.getSpaceConfig().getString("messages.prefix", "");
    ...
    /** Nombre de secondes écoulées depuis le début de la partie en cours (0 si aucune partie n'est en cours). */
    public long getElapsedSeconds() {
        if (gameStartTimeMillis <= 0L) return 0L;
        return Math.max(0L, (System.currentTimeMillis() - gameStartTimeMillis) / 1000L);
    }

    /** Nombre de zones capturées ("points marqués") depuis le début de la partie en cours. */
