...
public class Arena {
    ...
    /**
     * Limites (zone cuboïde) de chaque salle, par roomId. Optionnel : si aucune limite
     * n'est définie pour une salle, le confinement des spectateurs retombe sur un simple
     * rayon autour du point de vue (voir GameManager#confineSpectators). Définir les
     * limites précises d'une salle via /ss setroombounds permet un confinement fidèle à
     * la vraie forme de la pièce (murs, plafond...), pas juste un cercle approximatif.
     */
    private final Map<String, CuboidRegion> roomBounds = new LinkedHashMap<>();
    ...
    /**
     * Renvoie tous les points de spawn configurés, toutes salles et toutes équipes
     * confondues. Utilisé pour protéger la zone de spawn de chaque joueur (voir
     * {@link #isNearAnySpawn(Location, double)}) : on ne veut pas se limiter aux spawns
     * de la salle actuellement active, car les joueurs traversent plusieurs salles au
     * cours d'une même partie et chaque spawn doit rester dégagé en permanence.
     */
    public List<Location> getAllSpawnLocations() {
        List<Location> all = new ArrayList<>();
        for (Map<Team, List<Location>> room : roomSpawns.values()) {
            for (List<Location> list : room.values()) {
                all.addAll(list);
            }
        }
        return all;
    ...
    /**
     * True si {@code loc} se trouve à moins de {@code radius} blocs (distance 3D) d'au
     * moins un spawn configuré. Sert à empêcher de poser des blocs sur/autour des zones
     * de spawn (voir ArenaProtectionListener), pour qu'un joueur ne puisse jamais se
     * faire enfermer ou piéger dès son apparition.
     */
    public boolean isNearAnySpawn(Location loc, double radius) {
        if (loc.getWorld() == null) return false;
        double radiusSquared = radius * radius;
        for (Location spawn : getAllSpawnLocations()) {
            if (spawn.getWorld() == null || !spawn.getWorld().equals(loc.getWorld())) continue;
            if (spawn.distanceSquared(loc) <= radiusSquared) return true;
        }
        return false;
    }
    ...
    /** Définit (ou remplace) les limites de la salle {@code roomId}. */
    public void setRoomBounds(String roomId, CuboidRegion region) {
        roomBounds.put(roomId, region);
    }

    /** Renvoie les limites de la salle {@code roomId}, ou null si non configurées. */
    public CuboidRegion getRoomBounds(String roomId) {
        return roomBounds.get(roomId);
    }

    public void removeRoomBounds(String roomId) {
        roomBounds.remove(roomId);
    }
    ...
    public void saveToConfig(FileConfiguration config) {
        ...
        config.set("arena.roombounds", null);
        for (Map.Entry<String, CuboidRegion> e : roomBounds.entrySet()) {
            saveRegion(config, "arena.roombounds." + e.getKey(), e.getValue());
        }
    }
    ...
    public void loadFromConfig(FileConfiguration config) {
        ...
        roomBounds.clear();
        ConfigurationSection boundsSection = config.getConfigurationSection("arena.roombounds");
        if (boundsSection != null) {
            for (String roomId : boundsSection.getKeys(false)) {
                CuboidRegion region = loadRegion(config, "arena.roombounds." + roomId);
                if (region != null) roomBounds.put(roomId, region);
            }
        }
    }
