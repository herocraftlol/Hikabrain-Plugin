...
public class ArenaGUI {
    ...
    public Inventory buildInventory() {
        ...
        Collection<GameManager> allArenas = plugin.getSpaceArenaManager().getAll();
    ...
    private ItemStack buildArenaItem(GameManager gm, int maxPlayers) {
        ...
        if (spectatable && gm.getSpectatorCount() > 0) {
            lore.add(ChatColor.DARK_AQUA + "👁 " + gm.getSpectatorCount() + " spectateur" + (gm.getSpectatorCount() == 1 ? "" : "s"));
        }
    ...
    public String getArenaNameAt(int slot) {
        ...
        List<GameManager> list = new ArrayList<>(plugin.getSpaceArenaManager().getAll());
