...
import com.spaceship.plugin.hologram.LongestGamesLeaderboardManager;
...
public class SpaceShipCommand implements CommandExecutor, TabCompleter {
    ...
    // Stocke temporairement le coin 1 des limites d'une salle (pour le confinement des
    // spectateurs), par triplet (nom d'arène + roomId).
    private final Map<String, Location> pendingRoomBoundsCorner1 = new HashMap<>();

    public SpaceShipCommand(HikaBrainPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (sub) {
            ...
            case "setroombounds" -> handleSetRoomBounds(sender, args);
            case "delroombounds" -> handleDelRoomBounds(sender, args);
            ...
            case "longestgames" -> handleLongestGames(sender, args);
            // Scoreboard commands
    ...
    private void handleCreate(CommandSender sender, String[] args) {
        ...
        String name = args[1];
        boolean created = plugin.getSpaceArenaManager().create(name);
    ...
    private void handleDelete(CommandSender sender, String[] args) {
        ...
        String name = args[1];
        boolean deleted = plugin.getSpaceArenaManager().delete(name);
        MessageUtil.send(sender, deleted ? "&aArène '" + name + "' supprimée." : "&cAucune arène '" + name + "' trouvée.");
    }

    private void handleList(CommandSender sender) {
        ArenaManager am = plugin.getSpaceArenaManager();
    ...
    private GameManager resolveArena(CommandSender sender, String[] args, int nameIndex) {
        ...
        GameManager gm = plugin.getSpaceArenaManager().get(args[nameIndex]);
        if (gm == null) {
    ...
    private void handleArenasGui(CommandSender sender) {
        ...
        plugin.getSpaceArenaGUI().open(player);
    }
    ...
    private void handleLeave(CommandSender sender) {
        ...
        GameManager gm = plugin.getSpaceArenaManager().findArenaOf(player);
        if (gm == null) {
    ...
    private void handleUnspectate(CommandSender sender) {
        ...
        GameManager gm = plugin.getSpaceArenaManager().findSpectatingArenaOf(player);
        if (gm == null) {
    ...
    /**
     * /ss setroombounds <nom> <roomId> <pos1|pos2>
     * Définit les limites (zone cuboïde) d'une salle, utilisées pour confiner strictement
     * les spectateurs à l'intérieur (murs/plafond compris) — voir GameManager#confineSpectators.
     * Même mécanique en 2 clics que /ss setgoal / /ss setgamezone : viser un bloc à pos1,
     * viser le coin opposé à pos2 (portée de visée : 5 blocs, comme pour les buts).
     */
    private void handleSetRoomBounds(CommandSender sender, String[] args) {
        if (!checkAdminAndPlayer(sender)) return;
        if (args.length < 4) {
            MessageUtil.send(sender, "&cUsage: /ss setroombounds <nom> <roomId> <pos1|pos2>");
            return;
        }
        GameManager gm = resolveArena(sender, args, 1);
        if (gm == null) return;
        ...
        String pendingKey = args[1].toLowerCase(Locale.ROOT) + ":" + roomId;

        if (posArg.equals("pos1")) {
            pendingRoomBoundsCorner1.put(pendingKey, player.getTargetBlock(null, 5).getLocation());
            MessageUtil.send(sender, "&aCoin 1 des limites de la salle &7" + roomId
                    + " &aenregistré. Va au coin opposé (l'autre coin, en hauteur si besoin) puis : &7/ss setroombounds "
                    + args[1] + " " + roomId + " pos2");
        } else if (posArg.equals("pos2")) {
            Location corner1 = pendingRoomBoundsCorner1.get(pendingKey);
            if (corner1 == null) {
                MessageUtil.send(sender, "&cTu dois d'abord définir le coin 1 : &7/ss setroombounds " + args[1] + " " + roomId + " pos1");
                return;
            }
            Location corner2 = player.getTargetBlock(null, 5).getLocation();
            if (corner1.getWorld() == null || !corner1.getWorld().equals(corner2.getWorld())) {
            ...
            gm.getArena().setRoomBounds(roomId, new CuboidRegion(corner1, corner2));
            pendingRoomBoundsCorner1.remove(pendingKey);
            gm.saveArenaConfig();
            MessageUtil.send(sender, "&aLimites de la salle &7" + roomId + " &adéfinies ! Les spectateurs seront "
                    + "désormais bloqués (retéléportés au centre) s'ils sortent de cette zone.");
        } else {
            MessageUtil.send(sender, "&cUsage: /ss setroombounds <nom> <roomId> <pos1|pos2>");
        }
    }

    private void handleDelRoomBounds(CommandSender sender, String[] args) {
        if (!checkAdminAndPlayer(sender)) return;
        if (args.length < 3) {
            MessageUtil.send(sender, "&cUsage: /ss delroombounds <nom> <roomId>");
            return;
        }
        GameManager gm = resolveArena(sender, args, 1);
        if (gm == null) return;
        ...
        gm.getArena().removeRoomBounds(roomId);
        gm.saveArenaConfig();
        MessageUtil.send(sender, "&aLimites de la salle &7" + roomId + " &asupprimées (confinement approximatif par rayon utilisé à la place).");
    }
    ...
    private void handleSetSbServer(CommandSender sender, String[] args) {
        ...
        plugin.getSpaceScoreboardManager().setServerName(serverName);
    ...
    private void handleSetSbGame(CommandSender sender, String[] args) {
        ...
        plugin.getSpaceScoreboardManager().setGameName(gameName);
    ...
    private void handleSetSbTitle(CommandSender sender, String[] args) {
        ...
        plugin.getSpaceScoreboardManager().setTitle(title.toString());
    ...
    private void handleSetSbLines(CommandSender sender, String[] args) {
        ...
        plugin.getSpaceScoreboardManager().setLines(lines);
    ...
    private void handleReloadSb(CommandSender sender) {
        ...
        plugin.getSpaceScoreboardManager().reload();
    ...
    private void handleSbInfo(CommandSender sender) {
        ...
        MessageUtil.send(sender, "&7Titre: &f" + plugin.getSpaceScoreboardManager().getTitle());
        MessageUtil.send(sender, "&7Serveur: &f" + plugin.getSpaceScoreboardManager().getServerName());
        MessageUtil.send(sender, "&7Jeu: &f" + plugin.getSpaceScoreboardManager().getGameName());
        MessageUtil.send(sender, "&7Lignes: &f" + plugin.getSpaceConfig().getStringList("scoreboard.lines").size());
    ...
    private void handleStats(CommandSender sender, String[] args) {
        ...
        com.spaceship.plugin.stats.StatsManager.PlayerStats stats =
                plugin.getSpaceStatsManager().getPlayerStats(targetUuid, targetName);
        ...
        MessageUtil.send(sender, "&fParties jouées: &7" + plugin.getSpaceStatsManager().getTotalGames()
                + "  &fCaptures: &7" + plugin.getSpaceStatsManager().getTotalCaptures());
    ...
    private void handleTop(CommandSender sender, String[] args) {
        ...
        List<Map.Entry<UUID, com.spaceship.plugin.stats.StatsManager.PlayerStats>> top =
                plugin.getSpaceStatsManager().getTopPlayers(10, comparator);
    ...
    private void handleLeaderboard(CommandSender sender, String[] args) {
        ...
        CategoryLeaderboardManager lm = plugin.getSpaceLeaderboardManager();
    ...
    /**
     * /ss longestgames                          - affiche le top 10 des parties les plus longues dans le chat
     * /ss longestgames hologram                 - spawn l'hologramme à ta position (admin)
     * /ss longestgames hologram remove          - supprime l'hologramme (admin)
     * /ss longestgames hologram size <taille>   - règle la taille de l'hologramme (admin)
     */
    private void handleLongestGames(CommandSender sender, String[] args) {
        if (args.length >= 2 && args[1].equalsIgnoreCase("hologram")) {
            handleLongestGamesHologram(sender, args);
            return;
        }

        GameHistoryManager hm = plugin.getSpaceGameHistoryManager();
        List<GameRecord> top = hm.getLongestGames(10);

        MessageUtil.send(sender, "&8&m----------&r &6&l\u23f1 Parties les plus longues &8&m----------");

        if (top.isEmpty()) {
            MessageUtil.send(sender, "&7Aucune partie enregistrée pour le moment.");
        } else {
            int rank = 1;
            for (GameRecord record : top) {
                ...
                MessageUtil.send(sender, rankColor + "&l#" + rank + " &e" + record.getFormattedDuration()
                        + " &7- &f" + record.getFormattedDate() + " &7(" + record.getArenaName() + ")");
                MessageUtil.send(sender, "  &8Noir: &7" + joinOrNone(record.getBlackPlayers())
                        + " &f| &fBlanc: &7" + joinOrNone(record.getWhitePlayers()));
                MessageUtil.send(sender, "  &c\u2694 " + record.getTotalKills() + " kills"
                        + " &7- &b\u2691 " + record.getTotalPoints() + " points"
                        + " &7- Vainqueur: " + record.getWinner().getColoredName());
                rank++;
            }
        }
    ...
    private String joinOrNone(List<String> names) {
        return names.isEmpty() ? "(aucun)" : String.join(", ", names);
    }

    private void handleLongestGamesHologram(CommandSender sender, String[] args) {
        if (!sender.hasPermission("spaceship.admin")) {
            MessageUtil.send(sender, "&cTu n'as pas la permission.");
            return;
        }

        LongestGamesLeaderboardManager lm = plugin.getSpaceLongestGamesLeaderboardManager();

        if (args.length >= 3 && args[2].equalsIgnoreCase("remove")) {
            boolean removed = lm.despawn();
            MessageUtil.send(sender, removed
                    ? "&aHologramme 'parties les plus longues' supprimé."
                    : "&cAucun hologramme 'parties les plus longues' n'est actuellement actif.");
            return;
        }

        if (args.length >= 3 && args[2].equalsIgnoreCase("size")) {
            if (args.length < 4) {
                MessageUtil.send(sender, "&cUsage: /ss longestgames hologram size <taille>");
                MessageUtil.send(sender, "&7La taille est un multiplicateur (ex: 1.0 = normal, 2.0 = deux fois plus grand, 0.5 = deux fois plus petit). Doit être entre 0.1 et 10.");
                return;
            }
            if (!lm.isSpawned()) {
                MessageUtil.send(sender, "&cAucun hologramme 'parties les plus longues' n'est actuellement actif.");
                return;
            }
            double scale;
            try {
            ...
            lm.setScale(scale);
            MessageUtil.send(sender, "&aTaille de l'hologramme 'parties les plus longues' réglée sur " + scale + ".");
            return;
        }
        ...
        lm.spawn(player.getLocation());
        MessageUtil.send(sender, "&aHologramme 'parties les plus longues' (top 5) spawné à ta position !");
    }
    ...
    private void handleResetStats(CommandSender sender) {
        ...
        plugin.getSpaceStatsManager().resetStats();
    ...
    private void sendHelp(CommandSender sender) {
        ...
        MessageUtil.send(sender, "&e/ss longestgames &7- Voir le classement des parties les plus longues");
        if (sender.hasPermission("spaceship.admin")) {
            ...
            MessageUtil.send(sender, "&c/ss setroombounds <nom> <roomId> <pos1|pos2> &7- Définir les limites d'une salle (confine les spectateurs)");
            MessageUtil.send(sender, "&c/ss delroombounds <nom> <roomId> &7- Supprimer les limites d'une salle");
            ...
            MessageUtil.send(sender, "&b/ss longestgames hologram &7- Spawner l'hologramme des parties les plus longues à ta position");
            MessageUtil.send(sender, "&b/ss longestgames hologram remove &7- Supprimer cet hologramme");
            MessageUtil.send(sender, "&b/ss longestgames hologram size <taille> &7- Régler la taille de cet hologramme (ex: 1.5)");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            List<String> options = new ArrayList<>(List.of("join", "joinrandom", "leave", "spectate", "unspectate", "info", "list", "arenas", "stats", "top", "longestgames"));
            if (sender.hasPermission("spaceship.admin")) {
                options.addAll(List.of("create", "delete", "setlobby", "setzonecount", "setspawn", "delspawn", "setgoal", "setgamezone", "setmaxplayers", "setspecspawn", "setroombounds", "delroombounds", "start", "stop"));
        ...
        if (args.length == 2 && Set.of("join", "info", "delete", "setlobby", "setzonecount", "setspawn", "delspawn", "setgoal", "setgamezone", "setmaxplayers", "setspecspawn", "setroombounds", "delroombounds", "start", "stop", "spectate").contains(sub)) {
            return filterStartingWith(new ArrayList<>(plugin.getSpaceArenaManager().getNames()), args[1]);
        }

        if (args.length == 3 && sub.equals("setspecspawn")) {
            GameManager gm2 = plugin.getSpaceArenaManager().get(args[1]);
        ...
        if (args.length == 3 && (sub.equals("setroombounds") || sub.equals("delroombounds"))) {
            GameManager gm2 = plugin.getSpaceArenaManager().get(args[1]);
            List<String> rooms = gm2 != null ? buildRoomIdList(gm2)
                    : List.of("mid", "base1black", "base1white", "base2black", "base2white");
            return filterStartingWith(rooms, args[2]);
        ...
        if (args.length == 4 && sub.equals("setroombounds")) {
        ...
        if (args.length == 2 && sub.equals("longestgames")) {
            return filterStartingWith(List.of("hologram"), args[1]);
        }

        if (args.length == 3 && sub.equals("longestgames") && args[1].equalsIgnoreCase("hologram")) {
            return filterStartingWith(List.of("remove", "size"), args[2]);
        }

        if (args.length == 4 && sub.equals("longestgames") && args[1].equalsIgnoreCase("hologram") && args[2].equalsIgnoreCase("size")) {
            return filterStartingWith(List.of("0.5", "1.0", "1.5", "2.0", "3.0"), args[3]);
        }
