...
public class TournamentCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleCreate(CommandSender sender, String[] args) {
        ...
        TournamentManager.CreateResult result = plugin.getSpaceTournamentManager()
    ...
    private void handleJoin(CommandSender sender, String[] args) {
        ...
        TournamentManager.JoinResult result = plugin.getSpaceTournamentManager().join(args[1], player, teamTag);
    ...
    private void handleLeave(CommandSender sender, String[] args) {
        ...
        boolean left = plugin.getSpaceTournamentManager().leave(args[1], player);
    ...
    private void handleStart(CommandSender sender, String[] args) {
        ...
        TournamentManager.StartResult result = plugin.getSpaceTournamentManager().start(args[1]);
        switch (result) {
    ...
    private void handleCancel(CommandSender sender, String[] args) {
        ...
        boolean cancelled = plugin.getSpaceTournamentManager().cancel(args[1]);
        MessageUtil.send(sender, cancelled ? "&aTournoi '" + args[1] + "' annulé."
                : "&cAucun tournoi '" + args[1] + "' n'existe.");
    }

    private void handleList(CommandSender sender) {
        List<String> names = plugin.getSpaceTournamentManager().getNames();
        if (names.isEmpty()) {
        ...
        for (String name : names) {
            Tournament t = plugin.getSpaceTournamentManager().get(name);
            if (t == null) continue;
    ...
    private void handleInfo(CommandSender sender, String[] args) {
        ...
        Tournament t = plugin.getSpaceTournamentManager().get(args[1]);
        if (t == null) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 2 && List.of("join", "leave", "start", "cancel", "info", "bracket")
            ...
            return filterStartingWith(plugin.getSpaceTournamentManager().getNames(), args[1]);
        }
